...
public class GraphEvents extends TableEvents{
  ...
  public void getDistanceGraphEvent() {
    ...
    for (int i = 0; i < dataSize; i++) {
      ...
      if (temp[i][SET] != 0) {
        ...
        if (temp[i][ROUNDS] != 0) {
          tempValue[temp[i][SET]] += temp[i][ROUNDS] * temp[i][REPS] * temp[i][DISTANCE];
        } else if (temp[i][ROUNDS] == 0) {
          tempValue[temp[i][SET]] += temp[i][REPS] * temp[i][DISTANCE];
        }
      }
      // Checks for a calculated distance and for a type present
      if (tempValue[temp[i][SET]] != 0 && tempSet.getSetCol() != null 
          && !tempSet.getSetCol().contentEquals("")) {
        graphData.addData(tempSet.getSetCol(), tempValue[temp[i][SET]]);
      }
    }  
    updateGraph("distance", index, SET);
  }
  ...
  public void getIntensityGraphEvent() {
    ...
    int[] tempIntensity = new int[20];  //TODO
    int[] denom = new int[20];  //TODO
    try {
    ...
    updateGraph("intensity", index, SET);
  }

  /**
   * displays the amount of distance per type
   */
  public void getTypeGraphEvent() {
    GraphData graphData = MainPage.getGraphs().getData("type", index);
    int dataSize = MainPage.getTable(ID).getData().size();
    // Set, Rounds, Reps, Distance, Type
    boolean[] ttlDistance =
        new boolean[] {true, true, true, true, false, true, false, false, false};                                                                     // distance
    ...
    updateGraph("type", index, TYPE);
  }
  
  public void updateGraph(String name, int index, int colID) {
