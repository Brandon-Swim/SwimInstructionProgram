...
public class SelectionList {
  ...
  private final Border BLACK_BORDER = new Border(new BorderStroke(Color.BLACK, BorderStrokeStyle.SOLID,
      CornerRadii.EMPTY, BorderWidths.DEFAULT));
  private final int WIDTH = 500;
  private final Font TEST_FONT = new Font("Arial", 14);
  ...
  private Label[] info; // All of the information selected in the settings
  private Pane labelPane; //Pane to hold the of the info labels
  private VBox labelLayout; // Layout for all of the labels with in the labelPane
  ...
  public SelectionList() {
    ...
    info = new Label[Storage.sideLabel.length];
    ...
    selectorScrollPane.setPrefSize(WIDTH, 350);
    ...
    for (int i = 0; i < info.length; i++) {
      info[i] = new Label("Label: ");
      info[i].setFont(TEST_FONT);
      info[i].setBorder(RED_BORDER);
      info[i].setPrefSize(WIDTH, 25);
      labelLayout.getChildren().add(info[i]);
    }
    ...
    labelScrollPane.setPrefSize(WIDTH, 150);
